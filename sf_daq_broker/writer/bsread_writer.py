...
def write_from_databuffer_api3(data_api_request, output_file, parameters):
    ...
    try:
        bsread_data_api3_h5py = h5py.File(output_file,"r")
        inside_data_api3_file = list(bsread_data_api3_h5py.keys())
        for channel in channels:
            if channel not in inside_data_api3_file:
                _logger.error(f'check channel {channel} not present in data_api3 file')
            else:

                pulse_id_raw    = bsread_data_api3_h5py[f'/{channel}/pulse_id'][:]

...
def check_data_consistency(start_pulse_id, stop_pulse_id, rate_multiplicator, channels, output_file):
    ...
    try:
        ...
        for channel in channels:
            ...
            if start_matched_pulse_ids != start_expected:
                _logger.error(f"check {channel} start pulse ID {start_matched_pulse_ids} differs from requested {start_expected}")

            stop_matched_pulse_ids = matched_pulse_ids[-1] if len(matched_pulse_ids) else None
            stop_expected = expected_pulse_ids[-1]
            if stop_matched_pulse_ids != stop_expected:
                _logger.error(f"check {channel} stop pulse ID {stop_matched_pulse_ids} differs from requested {stop_expected}")

            if start_matched_pulse_ids == start_expected and stop_matched_pulse_ids == stop_expected and not has_duplicates:
                for pi, epi in zip(matched_pulse_ids, expected_pulse_ids):
                    if pi != epi:
